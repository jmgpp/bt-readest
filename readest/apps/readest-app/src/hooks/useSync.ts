...
export function useSync(bookKey?: string) {
  ...
  const pullChanges = async (
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        // Don't show error messages for expected offline mode errors
        if (!err.message.includes('Offline mode')) {
          setSyncError(err.message || `Error pulling ${type}`);
        } else {
          console.log('App is in offline mode, sync skipped');
        }
      } else {
  ...
  const syncBooks = async (books?: Book[], op: SyncOp = 'both') => {
    // Skip sync completely in these cases:
    // 1. When we're just pulling and there are no books
    // 2. When we're on web and not online
    // 3. When the user isn't authenticated
    
    if (op === 'pull' && !books?.length) {
      console.log('No books to sync, skipping pull operation');
      return;
    }
    ...
    try {
      // Check if we have a token - if not, don't even try syncing
      ...
      console.log('Error checking authentication, skipping sync', error);
      return;
    }

    // Now proceed with the actual sync operations
